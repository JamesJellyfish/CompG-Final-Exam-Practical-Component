...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		//Int variables for toggle buttons
		int		  lightingOff = 0;
		int		  ambientLightOnly = 0;
		int		  specularLightOnly = 0;
		int		  ambientSpecular = 0;
		int		  ambientSpecularDoF = 0;
		int		  textures = 1;
		...
		shader->SetUniform("u_Lightoff", lightingOff);
		shader->SetUniform("u_Ambient", ambientLightOnly);
		shader->SetUniform("u_Specular", specularLightOnly);
		shader->SetUniform("u_AmbientSpecular", ambientSpecular);
		shader->SetUniform("u_AmbientSpecularDof", ambientSpecularDoF);
		shader->SetUniform("u_Texture", textures);
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			//Refence The Toggle Button from Our GDW game.
			if (ImGui::CollapsingHeader("Toggle buttons")) {
				if (ImGui::Button("No Lighting")) {
					shader->SetUniform("u_Lightoff", lightingOff = 1);
					shader->SetUniform("u_Ambient", ambientLightOnly = 0);
					shader->SetUniform("u_Specular", specularLightOnly = 0);
					shader->SetUniform("u_AmbientSpecular", ambientSpecular = 0);
					shader->SetUniform("u_Texture", textures = 4);
				}

				if (ImGui::Button("Ambient Light only")) {
					shader->SetUniform("u_Lightoff", lightingOff = 0);
					shader->SetUniform("u_Ambient", ambientLightOnly = 1);
					shader->SetUniform("u_Specular", specularLightOnly = 0);
					shader->SetUniform("u_AmbientSpecular", ambientSpecular = 0);
					shader->SetUniform("u_Texture", textures = 4);
				}

				if (ImGui::Button("Specular Light only")) {
					shader->SetUniform("u_Lightoff", lightingOff = 0);
					shader->SetUniform("u_Ambient", ambientLightOnly = 0);
					shader->SetUniform("u_Specular", specularLightOnly = 1);
					shader->SetUniform("u_AmbientSpecular", ambientSpecular = 0);
					shader->SetUniform("u_Texture", textures = 4);
				}
				...
				if (TextureToggle) {
					if (ImGui::Button("Turn Textures Off"))
					{
					...
					if (ImGui::Button("Turn Textures On"))
					{
						shader->SetUniform("u_Texture", textures = 1);
						TextureToggle = true;
					}
				}
			}
		...
		Texture2D::sptr grass = Texture2D::LoadFromFile("images/A2Grass.png");
		...
		Texture2D::sptr Hedge = Texture2D::LoadFromFile("images/Hedge.png");
		Texture2D::sptr Pathway = Texture2D::LoadFromFile("images/Pathway.png");
		Texture2D::sptr Slide = Texture2D::LoadFromFile("images/Slide.png");
		Texture2D::sptr RoundAbout = Texture2D::LoadFromFile("images/RoundAbout.png");
		Texture2D::sptr SandBox = Texture2D::LoadFromFile("images/SandBox.png");
		...
		ShaderMaterial::sptr hedgeMat = ShaderMaterial::Create();
		hedgeMat->Shader = shader;
		hedgeMat->Set("s_Diffuse", Hedge);
		hedgeMat->Set("s_Specular", noSpec);
		hedgeMat->Set("u_Shininess", 2.0f);
		hedgeMat->Set("u_TextureMix", 0.0f);

		ShaderMaterial::sptr pathwayMat = ShaderMaterial::Create();
		pathwayMat->Shader = shader;
		...
		ShaderMaterial::sptr SlideMat = ShaderMaterial::Create();
		SlideMat->Shader = shader;
		SlideMat->Set("s_Diffuse", Slide);
		SlideMat->Set("s_Specular", noSpec);
		SlideMat->Set("u_Shininess", 2.0f);
		SlideMat->Set("u_TextureMix", 0.0f);

		ShaderMaterial::sptr RoundAboutMat = ShaderMaterial::Create();
		RoundAboutMat->Shader = shader;
		...
		ShaderMaterial::sptr SandBoxMat = ShaderMaterial::Create();
		SandBoxMat->Shader = shader;
		SandBoxMat->Set("s_Diffuse", SandBox);
		SandBoxMat->Set("s_Specular", noSpec);
		SandBoxMat->Set("u_Shininess", 2.0f);
		SandBoxMat->Set("u_TextureMix", 0.0f);

		GameObject obj1 = scene->CreateEntity("Ground"); 
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/A2Ground.obj");
			...
			obj1.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
		}

		GameObject obj2 = scene->CreateEntity("Pathway");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/A2Pathway.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(pathwayMat);
			obj2.get<Transform>().SetLocalPosition(0.0f, 0.0f, -1.5f);
			obj2.get<Transform>().SetLocalRotation(90.0f, 0.0f, -90.0f);
			obj2.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}

		GameObject obj3 = scene->CreateEntity("Hedge");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/A2Hedge.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(hedgeMat);
		...
		GameObject obj4 = scene->CreateEntity("Slide");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/A2Slide.obj");
			obj4.emplace<RendererComponent>().SetMesh(vao).SetMaterial(SlideMat);
			obj4.get<Transform>().SetLocalPosition(0.0f, 0.0f, -2.0f);
			obj4.get<Transform>().SetLocalRotation(90.0f, 0.0f, -90.0f);
			obj4.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
		}

		GameObject obj5 = scene->CreateEntity("SandBox");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/A2SandBox.obj");
			obj5.emplace<RendererComponent>().SetMesh(vao).SetMaterial(SandBoxMat);
			obj5.get<Transform>().SetLocalPosition(0.0f, 0.0f, -1.5f);
			obj5.get<Transform>().SetLocalRotation(90.0f, 0.0f, -90.0f);
			obj5.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
		}

		GameObject obj6 = scene->CreateEntity("RoundAbout");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/A2RoundAbout.obj");
			obj6.emplace<RendererComponent>().SetMesh(vao).SetMaterial(RoundAboutMat);
			obj6.get<Transform>().SetLocalPosition(0.0f, 0.0f, -2.0f);
			obj6.get<Transform>().SetLocalRotation(90.0f, 0.0f, -90.0f);
			obj6.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
		}
